...
def fetch_latest_alerts(limit: int = 100) -> pd.DataFrame:    """Fetch latest alerts from SQLite database."""
    ...
    try:
        ...
        df = pd.read_sql_query(
            f"""
            SELECT id, timestamp, src_ip, dst_ip, dst_port, attack_type, confidence            FROM alerts
...
def main():
    ...
    # Create tabs
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([        "📊 Dashboard", 
        "🚨 Alerts", 
        "🕸️ Attack Chain", 
        "🐝 Honeypot",
        "📈 UEBA",
    ...
    # Load data
    alerts_db = fetch_latest_alerts(limit=100)    alerts_csv = load_alert_csv()
    ...
    with tab1:
        ...
        with col4:            if not alerts_db.empty and "confidence" in alerts_db.columns:
        ...
        with col1:
            ...
            if not alerts_db.empty and "timestamp" in alerts_db.columns:
                ...
                if not alerts_db_copy.empty:
                    ...
                    st.info("No valid timestamps in data")            else:
    ...
    with tab3:
        ...
        if not alerts_csv.empty:
            if "src_ip" in alerts_csv.columns and "dst_port" in alerts_csv.columns:
                ...
                st.dataframe(attack_chain, width='stretch', hide_index=True)            else:
